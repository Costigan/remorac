...
1. **Static shapes only** — dimensions from HIR type annotations.
1. **Descriptor ABI** — GPU kernels use Remora descriptor structs
   (aligned pointer + offset + sizes + strides) for input/output.
1. **File-naming**: `_` prefix for internal helpers, `lowering/` subdirectory
   for MLIR lowering modules.
1. **Testing**: pytest, `uv run pytest tests/`. GPU tests need IREE.
   CPU compiled tests use `CPUFunctionExecutor.compile_source()`.
1. **Naming in HIR**: `HIRMap`, `HIRFold`, `HIRReduce`, `HIRIndex`,
