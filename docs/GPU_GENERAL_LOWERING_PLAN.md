...
| Indexing | `HIRIndex` with scalar indices → strided load from descriptor |
| Reductions | `HIRFold` / `HIRReduce` with scalar result → per-thread inner loop |
| Bindings | `HIRLet` chains → sequentially lower value, bind, lower body |
| Constants | `HIRLit` → `llvm.mlir.constant` |
| Variables | `HIRVar` → lookup in input-mapping or scalar-kernel-param env |
...
- Multi-dimensional `HIRFold`/`HIRReduce` (array-valued fold result)
...
- [ ] `GpuInputLoad(index, coords)` — load from input descriptor at multi-dim coords
- [ ] `GpuConstant(value, element_type)` — float or int literal
- [ ] `GpuBinaryOp(op, left, right)` — `add`/`sub`/`mul`/`div` on `f32` (plus `i32` variants)
- [ ] `GpuCompareOp(op, left, right)` — `lt`/`gt`/`lte`/`gte`/`eq`/`neq` → `i1`
- [ ] `GpuSelect(cond, true_val, false_val)` — `llvm.select %cond, %t, %f`
- [ ] `GpuCast(expr, from_type, to_type)` → `llvm.sitofp` / `llvm.fptosi` / `llvm.trunc` / `llvm.sext`
- [ ] `GpuReduce(op, init, body, dimension)` — per-thread inner `scf.for` loop
- [ ] `GpuScalarParam(index)` — scalar kernel parameter
- [ ] `GpuVariable(name)` — let-bound SSA name

#### 1.2 — HIR → GPUExpr compiler

- [ ] `_gpu_expr_from_hir(expr, input_map, scalar_env, context)` — recursive compiler:
  ...
  | `HIRIf` | `GpuSelect(cond, then_val, else_val)` |
  | `HIRIndex` | strided `GpuInputLoad` using descriptor stride fields |
...
- [ ] `_gpu_emit_expr(expr, lines, env)` in `gpu_lowering.py`:
...
- [ ] `_gpu_descriptor_info(rank, prefixes)` — returns SSA names for aligned
      pointer, offset, per-dim sizes, and per-dim strides for each input/output.

#### 1.4 — Kernel scaffold

- [ ] `build_descriptor_abi_general_map_gpu_module(function, kernel_name)` in `gpu_lowering.py`:
...
- [ ] Add dispatch case in `codegen.py` `generate_mlir_descriptor_abi_ptx`,
...
- [ ] Catch only `GPUScaffoldError` and `CodegenUnavailable` so bugs surface.

### Phase 3 — Testing

- [ ] **Verify N-body** compiles and runs on GPU through the general path
      (not the specialised kernel).  Do NOT remove the specialised kernel
      until this passes.

- [ ] **Add GPU compilation tests** for structurally different programs:
  - Map with fold body: `map (lambda i → fold + 0 (map …))` (f32)
  - Map with index expression: `map (lambda i → (index pos i))` (f32)
  - Map with `::`-let scalar capture (f32)
  - Binary map with non-trivial sub-expression (f32)
  - Map with `HIRCast` (int→float, float→int)
  - Map with boolean condition via comparison + `HIRIf`
  - Integer map: `map (lambda i → …)` on `i32` input
  - Map using non-contiguous input via stride (if runtime supports it)

- [ ] **Add numeric parity tests** — for each test program, run both
      `CPUFunctionExecutor` and `RemoraExecutor`, assert outputs match
      within tolerance.

- [ ] Confirm all existing GPU tests (`test_executor.py -k gpu`) pass.

- [ ] Run full CPU regression (≈170 tests).

### Phase 4 — Cleanup

- [ ] Delete `_detect_nbody_gpu_kernel()` and
      `build_descriptor_abi_nbody_gpu_module()` from `gpu_lowering.py`.
- [ ] Remove the N-body dispatch block from `codegen.py`.
- [ ] Update `test_nbody.py` to use the general GPU path (remove
      N-body-specific kernel metadata assertions).
- [ ] Search for remaining example-specific code in the GPU path.

### Phase 5 — Math intrinsics

- [ ] Support `llvm.intr.sqrt` via declared `llvm.func` inside the module.
- [ ] If `exp`/`log` are needed, add with documented limitations
      (approximation or explicit declaration).
...
2. At least 5 structurally different programs compile and produce correct
   ...
   map-with-cast, and either map-with-condition or i32-map.
3. All existing CPU tests (≈170) and GPU tests (≈14) pass.
